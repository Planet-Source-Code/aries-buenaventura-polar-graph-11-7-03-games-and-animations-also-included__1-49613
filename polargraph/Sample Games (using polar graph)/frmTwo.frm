...
Dim BulletW    As Integer   ' Bullet width
...
Private Sub Render()
    ...
    If IsShipFire Then
        ...
        Amplitude = Amplitude + sldGameSpeed.Value + 5
    End If
    ...
    lblDetails(1).Caption = lblDetails(1).Tag & "A/(5 + Sin(t))"
    lblDetails(2).Caption = lblDetails(2).Tag & "0°"
    lblDetails(3).Caption = lblDetails(3).Tag & "180°"
    lblDetails(4).Caption = lblDetails(4).Tag & "10"
...
Private Sub ShipFire(ByVal Amp As Single)
    ...
    For Angle = 0 To 180 Step 10
        ...
        px = (Amp / (5 + Sin(Radian))) * Cos(Radian)
        py = (Amp / (5 + Sin(Radian))) * -Sin(Radian)
       
        px = sx + (WinW - BulletW) / 2 + px
        py = sy + (WinH - BulletH) / 2 - Ship.Height / 2 + py - 4
